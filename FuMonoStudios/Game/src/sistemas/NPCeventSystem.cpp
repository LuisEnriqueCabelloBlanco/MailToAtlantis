					...
					if (paquetesArray != currentEvent.end())
					{
						...
						if (obj.find("1") != obj.end())
						{
							for (auto paq : obj) {
								...
								createSpecificPaquete(paqObj, auxEvent);
							}
						}
						else // SI LOS DATOS SON PARA TODOS LOS PAQUETES
						{
							...
							{
								createSpecificPaquete(obj, auxEvent);
							}
						}
					}
					else
					...
					if (condicionesArray != currentEvent.end())
					{
						...
						if (obj.find("1") != obj.end())
						{
							...
							for (auto paq : obj)
							{
								...
								checkSpecificPaquete(pqConditions, auxEvent, i);
					...
					if (condicionesArray != currentEvent.end())
					{
						...
						if (obj.find("1") != obj.end())
						{
							...
							for (auto paq : obj)
							{
								...
								auxEvent->usingDifConditions = true;
								k++;
							}
						}
						else // SI LAS CONDICIONES SON PARA TODOS LOS PAQUETES
						{
							...
							auxEvent->usingDifConditions = false;
							auxEvent->condiciones.push_back(condicionesDeTodos);
						}
					}
					else
...
void NPCeventSystem::createSpecificPaquete(JSONObject paqObj, NPCevent* auxEvent) {
...
void NPCeventSystem::checkSpecificPaquete(JSONObject pqConditions, NPCevent* auxEvent, int i) {
